...
bool GraphicsClass::Frame()
{
	...
	{
		m_TextureShader->SwapFilter(m_D3D->GetDevice(), 1);
	}

	// Press Number 2
	if (m_Input->IsKeyDown(0x32))
	{
		m_TextureShader->SwapFilter(m_D3D->GetDevice(), 2);
	}

	// Press Number 3
	if (m_Input->IsKeyDown(0x33))
	{
		m_TextureShader->SwapFilter(m_D3D->GetDevice(), 3);
	}

	// Press Number 4
	if (m_Input->IsKeyDown(0x34))
	{
		m_TextureShader->SwapFilter(m_D3D->GetDevice(), 4);
	}
	...
	{
		m_Camera->MoveCamera(true);
	}

	// Press W
	if (m_Input->IsKeyDown('S'))
	{
		m_Camera->MoveCamera(false);
	}
...
void GraphicsClass::SwapTexture(int num)
{
	//m_TextureShader->SwapFilter(num);
}